...
const AppNavBar = ({ user, history, showDrawer, showWindow, theme, location }) => {
  if (
    (location && location.pathname.indexOf('/full') !== -1) ||
    (location && location.pathname.indexOf('/print') !== -1)
  ) {
    return null;
  }
  ...
  const pathIndex = (Modules.getAppMenuItemList() || [])
    .filter((item) => !item.isProtected || (user && user.roles.indexOf('Publico') === -1))
    .findIndex(
      (menuData) =>
        (menuData.path === '/' && history.location.pathname === '/') ||
        (menuData.path !== '/' &&
          history.location &&
          ...
          <div style={appNavBarStyle.containerNavBar}>
            <div style={appNavBarStyle.subContainerNavBar}>
              <Tabs aria-label="icon label tabs example">
                {(Modules.getAppMenuItemList() || [])
                  .filter(
                    (item) => !item.isProtected || (user && user.roles.indexOf('Publico') === -1),
                  )
                  .map((menuData, ind) => (
                    <Button
                      style={{
