...
export default function SimpleTable({
  ...
  const renderType = (type: string, data: any, colName: string) => {
    ...
    if (type === 'image') {
          ...
          {!data
            ...
            : data.map((item, index, array) => {
                if (typeof item === 'string') {
                  return <Chip variant="outlined" label={item} color={'primary'} />;
      ...
      return Array.isArray(data) ? (
        data.map((d) => <div dangerouslySetInnerHTML={{ __html: d }} />)
      ) : (
      ...
      const schemaData =
        ...
        schema[colName].options.find((object) => object.value === data || object === data);
  ...
  const filterList = {};
  const tableBody = (filter ? newDate.filter((d) => d[filterByField] === filter) : newDate).map(
    (row, index) => {
      if (filterByField) {
        ...
        >
          {cols.map((col) => (
            ...
            >
              {col.type === 'text' &&
              row[col.field] &&
              renderType(col.type, row[col.field], col.field).length > 35 ? (
      ...
      {filterByField ? (
        ...
        >
          ...
          {filter ? (
            ...
            >
              <MenuItem key={'NoFilter'} value={null}>
                {'Sem Filtro'}
              </MenuItem>
