...
class SimpleForm extends Component<ISimpleFormProps> {
    ...
    if (element.props.formType === 'subform' && !!element.props.name) {
      return (
        <SubFormComponent
          ...
          initialValue={
            self.props.doc
              ? self.props.doc[element.props.name]
  ...
  validate = () => {
    ...
    if (fielsWithError.length > 0) {
      showNotification({
        ...
        description: `Os seguintes campos são obrigatórios e precisam ser preenchidos: ${
          fielsWithError.join(', ').replaceAll('*', '') + '.'
        }`,
        durations: 8000,
      });
  ...
  render() {
    ...
    return <div style={this.props.style || { width: '100%' }}>{this.formElements}</div>;
  }
}
