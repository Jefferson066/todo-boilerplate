...
export default (props) => {
  ...
  if (!!screenState && validState.indexOf(screenState) !== -1) {
    if (screenState === 'view' && !!id) {
      return <UserProfileDetailContainer {...props} screenState={screenState} id={id} />;
    } else if (screenState === 'edit' && !!id) {
      return <UserProfileDetailContainer {...props} screenState={screenState} id={id} edit />;
    } else if (screenState === 'create') {
      return <UserProfileDetailContainer {...props} screenState={screenState} create />;
    }
  } else {
