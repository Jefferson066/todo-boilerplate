    ...
    >
      ...
      >
        ...
        <FormGroup>
          ...
          <TextField placeholder="Empresa" name="empresa" />
          <DatePickerField placeholder="Data de Nascimento" name="birthDate" />
          <SelectField
            placeholder="Sexo"
            options={[
        ...
        <div key={'Buttons'} style={{ paddingTop: 20, paddingBottom: 20 }}>
          <Button
            onClick={
              screenState === 'edit'
                ? () => history.push(`/userprofile/view/${user._id}`)
                : !!viewer
                ? close
                : () => history.push(`/userprofile/list`)
            }
            color={'secondary'}
            variant="contained"
          >
